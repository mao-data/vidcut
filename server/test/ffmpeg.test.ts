...
describe('ffmpeg wrapper', () => {
  ...
  it('probe 附帶 codec/pixFmt/container/keyframeIntervalSec（有視訊的檔案）', async () => {
  ...
  it('probe：短片＋預設長 GOP（60 秒窗內僅 1 個 keyframe）時 keyframeIntervalSec 缺席', async () => {
    const dir = await tmpDir('vidcut-ff-');
    const f = await makeVideo(dir, 'a.mp4', { duration: 3, withAudio: true });
    const p = await probe(f);
    expect(p.codec).toBe('h264');
  ...
  it('probe：純音訊檔沒有 keyframeIntervalSec（undefined）', async () => {
    const dir = await tmpDir('vidcut-ff-');
