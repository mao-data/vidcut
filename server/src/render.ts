...
export async function render(
  ...
  // 刻意排在補測 audioChannels 之前：那一步的 catch 會吞掉錯誤，先擋才有明確訊息。
  const missing = stored.media
    .filter(
      (m) =>
        stored.tracks.video.some((c) => c.mediaId === m.id) ||
        stored.tracks.audio.some((a) => a.mediaId === m.id),
    )
...
export async function renderCoverImage(
  ...
  if (lastAbs && existsSync(lastAbs)) {
    // 已有成品：所見即所得（含 overlay/字幕，全部已經燒進成片），直接抽幀即可。
    await runFfmpeg([
      '-ss',
      String(time),
      '-i',
  ...
  // frozen clip 定格在單一來源時刻 clip.in；一般片段線性映射 in + offset。
