...
- ⚠️ **karaoke 字幕**：預覽是「base 卡 + 全高亮卡疊 `clip-path`」，匯出是「**一個詞一張卡**」，
  ...
  **先講結論，免得下一個人跑去修一個不存在的 bug：匯出那條路是正確的**（一詞一卡＝單層，
  每個字直接用自己的顏色畫，沒有滲色也沒有疊層）。有偏差的是**預覽**，症狀是描邊看起來
  略厚一點點。**不是成品壞掉**，使用者看匯出的影片不會發現任何問題。

  卡片層級實測（4 詞 CJK「這一句/有/逐詞/高亮」，64px、有描邊、1080×92 的卡）：

  | 高亮到 | 不同像素 | 佔比  | 最大單通道差 | 差異 x 範圍 |
  | ------ | -------- | ----- | ------------ | ----------- |
  | 1 詞   | 772      | 0.78% | 104          | 283~480     |
  | 2 詞   | 1101     | 1.11% | 165          | 283~544     |
  | 3 詞   | 1664     | 1.68% | 64           | 283~672     |
  | 4 詞   | 2227     | 2.24% | 64           | 283~796     |

  （舊文字寫「6 詞、793–2764 個像素、最大單通道差 255」——像素數量級是對的，
  但**最大單通道差實測是 104–165，不是 255**。差異像素散佈在**整段已高亮文字的描邊
  反鋸齒邊緣**，不是集中在 `pad` 邊界那一條；也就是成因 (b) 才是主要貢獻者，(a) 只佔
  邊界那 2px 寬的十幾個點。）

  ⚠️ **要重測的話，合成模型不能用 PIL 的 `paste(mask=)`** —— 那是線性內插（取代），
  瀏覽器兩張 `<img>` 疊起來是 **source-over（alpha 累積）**。用 paste 量會得到 10–19 個
