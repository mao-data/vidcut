...
它*不是* prompt 生影片的產生器。AI agent（Claude Code 或任何 MCP client）透過 [34 個 MCP 工具](#mcp-工具)剪片——匯入素材、粗剪、上字幕、混音、輸出。每一步變更都經 WebSocket **即時出現在你的瀏覽器**。你拖一下字幕、修一刀 trim、留一句審核意見——AI 讀回你的調整、接著做。這是人在迴路中的剪輯協作，不是黑盒子。
