...
vidcut 是一個 **本機優先的短影音時間軸編輯器**，畫布有四檔比例 preset（預設直式 1080×1920，另有橫式 16:9、方形 1:1、直式 4:5），核心理念只有一句話：**AI 跟人共用同一條時間軸、同時編輯**。

它*不是* prompt 生影片的產生器。AI agent（Claude Code 或任何 MCP client）透過 [39 個 MCP 工具](#mcp-工具)剪片——匯入素材、粗剪、上字幕、混音、輸出。每一步變更都經 WebSocket **即時出現在你的瀏覽器**。你拖一下字幕、修一刀 trim、留一句審核意見——AI 讀回你的調整、接著做。這是人在迴路中的剪輯協作，不是黑盒子。
...
- 🎬 **真實渲染管線** —— ffmpeg 從 `project.json` 依專案畫布尺寸合成成品，含進度回報。
