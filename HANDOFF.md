...
> 四階段之後 `main` 又併入了數個完整功能（素材匯入零複製、MCP 面補完、預覽音訊同步、
> AI 編輯動畫、面板寬度拖曳、字幕匯出四模式、真 redo stack）——見下面現況總覽的表。
>
...
> 卡片層級量測，**沒有常駐測試守著、未重驗**）。範圍與完整實測表見 `CLAUDE.md`「『預覽即成品』的
...
| MCP 面補完            | ✅                                             | `server/src/mcp.ts` 現在是 **31 個 `registerTool`、每個都宣告 `outputSchema`**（`server/test/mcp-tools.test.ts` 的「批 G」守著）；相對於 M3 新增 `add_clip`／`remove_overlay`／`remove_audio`／`update_caption`／`list_source`／`import_media`／`redo`。「工具有沒有漏註冊、instructions 有沒有跟上」由 `server/test/mcp-docs-sync.test.ts` 守著（鐵則第三步的執行面）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
