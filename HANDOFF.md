...
> 見 `ui/e2e/preview-vs-export.mjs` 的 `CASES`；常駐容差是腳本裡的 `TOL_PX` = 4）。
> **2026-08-26 起是直式＋橫式雙基線**：`portrait` 6/6 最大差 1.0px、`landscape`
> （`VIDCUT_CANVAS=landscape`）6/6 最大差 1.2px；**`square` 與 `portrait-4-5` 沒跑過**，
> 那兩檔的「預覽即成品」是推得的不是量出來的。
...
6. **預覽=成品的最終檢驗（階段 4 收尾）**：把上面拖過的 overlay/字幕、改過的文字，用同一次渲染比對。
   **請帶著下面這份「已知會不一致」的清單去看**，否則你會把已知落差當成新 bug（或反過來，
   ...
   - ✅ **overlay 的大小/位置現在也應該一樣**（2026-08-04 修好兩個幾何落差；`verify:wysiwyg`
     ...
     `TOL_PX` = 4；2026-08-26 雙基線實測：直式 6/6 最大 1.0px、橫式 6/6 最大 1.2px）：
...
ui/src/panels/MediaPanel/ 右欄 Media 分頁（asset library phase 2 UI，見上方里程碑表）：
                          `index.tsx` 分頁殼（Project/Library/Source 三顆 zone 鈕，`useState`
                          切換，無 URL 路由）＋ `ProjectMediaZone.tsx`（列出當前專案素材、加入
                          時間軸、存入素材庫）／`LibraryZone.tsx`（搜尋、上傳、匯入、雙擊改
                          標籤、刪除需二次確認）／`SourceFolderZone.tsx`（掃描路徑、勾選批次
                          匯入、直接入庫）。三檔皆零 `useSelection.select()`——App.tsx 的
                          「選取跳 Properties」effect 會把使用者踢出本分頁，面板刻意繞開它。
...
npm run verify:wysiwyg # 真 render vs 預覽截圖的墨跡外框（自己起 :3999 + 臨時專案；六個 case 全綠。
                       # 畫布比例吃 VIDCUT_CANVAS，直式＋橫式各有一條基線）
