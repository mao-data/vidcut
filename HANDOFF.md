...
| MCP 面補完            | ✅                                             | **每個工具都宣告 `outputSchema`**，由 `server/test/mcp-tools.test.ts` 的「批 G」守著（工具**數量**以 `server/src/mcp.ts` 為準——那條測試是 `toBeGreaterThanOrEqual`，下限不是釘死值，所以這裡不寫死）。相對於 `m3-done` 的 15 個工具，新增 `add_clip`／`remove_overlay`／`remove_audio`／`update_caption`／`list_source`／`redo`（`import_media` **M3 就有**，之後改的是語意——擴充成絕對路徑零複製引用）。「工具有沒有漏註冊、instructions 有沒有跟上」由 `server/test/mcp-docs-sync.test.ts` 守著（鐵則第三步的執行面）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
