...
- **karaoke（預覽端）**：base 卡 + 全高亮卡（`.hl.png`）兩張**幾何完全相同**的圖疊在一起，上層用 `shared/src/captions.ts` 的純函數 `karaokeClip(bboxes, activeIndex, pad)` 算出的 `clip-path` 逐詞揭色（`pad` 補償描邊外擴）；`tokenSeparator(prev, next)` 判斷詞間該不該插空白（CJK 不加、拉丁加），DOM fallback 與伺服端 `text_card.py` 的斷詞規則因此一致。**匯出端維持階段 1 就有的「一個詞一張卡」機制沒有變**（`server/src/render.ts` 的 `renderCaptionCards`）——兩卡+clip-path 目前只在預覽端，渲染端的「一詞一卡爆量」根治仍是後續工作（spec §2 非目標）。
  ...
  比單卡厚。
  **哪一邊是對的：匯出**（一詞一卡＝單層，每個字直接用自己的顏色畫，沒有滲色也沒有疊層）。
  預覽是描邊略厚的那一邊。**不是成品壞掉**——使用者看匯出的影片不會發現任何問題，
  別把這條當成「karaoke 有 bug」去修。
  卡片層級實測（2026-08-05 重測，4 詞 CJK「這一句/有/逐詞/高亮」，64px、有描邊、卡 1080×92）：
  1 詞 772 個像素／0.78%、2 詞 1101／1.11%、3 詞 1664／1.68%、4 詞 2227／2.24%；
  最大單通道差 **104–165**（舊文字寫 255，實測重現不出來）。差異像素散佈在**整段已高亮文字的
  描邊反鋸齒邊緣**（x 範圍隨高亮長度一起長），所以主要貢獻者是 (b) 而不是 (a)——
  (a) 只佔 `pad` 邊界那 2px 寬的十幾個點。
  ⚠️ **重測時合成模型不能用 PIL 的 `paste(mask=)`**（線性內插＝取代），瀏覽器兩張 `<img>` 疊起來是
  **source-over（alpha 累積）**。用 paste 量會得到 10–19 像素的假結果，正好把成因 (b) 整個抹掉，
  看起來像「幾乎沒差」。正確做法：hl 的 alpha 乘上 clip 遮罩後 `Image.alpha_composite(base, clipped)`。
  修法要嘛匯出端也改走兩卡+clip-path，要嘛預覽端改成一次只畫一張「第 k 個詞高亮」的卡——兩條都還沒做，
  而且以目前量到的偏差幅度，這件事的優先度應該排在功能後面。
...
- ~~**`OverlayText.maxWidth` 是死欄位**~~ **2026-08-04 已實作**：`text_card.py` 的無 tokens 路徑現在也會折行，`maxWidth` 真的控制換行寬。細節見下方「文字自動換行」節。
...
## 文字自動換行（2026-08-04，分支 `caption-wysiwyg`）

`OverlayText.maxWidth` 從**死欄位**變成真的生效。以前 `text_card.py` 只在 `layout_tokens()`
（＝只有 karaoke 字幕）裡折行，文字 overlay 與一般字幕走 `text.split("\n")`——**完全不換行，
太長的字被畫布邊緣裁掉且沒有任何警告**（同一段長文字給 0.9 與 0.3 產出的 PNG sha256 相同）。

- **換行規則**（`text_card.py` 的 `wrap_text()` / `split_atoms()` / `break_overwide()`）：
  可用寬 = `width - cardMargin(width, maxWidthFrac) * 2`；CJK 逐字折、拉丁整個單字為單位
  （不切進單字中間）、換行點的空白丟掉、行首禁則標點（。，」）…）黏回前一行、
  真的 `\n` 仍強制換行、單一超寬不可斷字串**逐字硬切**（等同 CSS `break-word`，
  保證收斂且不溢出）。**沒做**「開頭引號不得置於行尾」（需要往後預讀，刻意留白）。
- **`cardMargin()` 是唯一的換行寬換算來源**（`server/src/rasterizer.ts`）。匯出路徑
  （`render.ts` 自己 spawn 的 CLI）以前**不傳 `margin`**，靠 python 預設 `max(32, width//20)`
...
- **副作用（預期內，不是回歸）**：既有專案裡「以前被裁掉」的長文字現在會折行 → 字卡變高、
  hash 改變（內容定址，舊 PNG 變孤兒檔，`derived/text/` 目前只增不減）。
  `projects/demo` 的 `try_text`（「拖我 213」，fontSize 200、maxWidth 0.85）實測寬 783
  < 可用寬 918 → **仍是一行、PNG 位元組完全相同**，demo 專案不受影響。
- **文件同步**：`shared/src/types.ts` 的 `OverlayText.maxWidth` 註解、`server/src/rasterizer.ts`
  的 `maxWidthFrac` 註解、`server/src/mcp.ts` 的 `overlayTextSchema.maxWidth`（以前**完全沒有
...
- **做法**：自己在 `os.tmpdir()/vidcut-wysiwyg-fixture` 建一個臨時專案（純深灰 1080×1920
  影片 4 秒、白字深描邊、四個項目各佔一段互不重疊的時間窗），自己在 :3999 起一台 server
...
- **六個 case、目前的實測結果（六項全綠）**：
  ...
  - ✅ **文字 overlay 的自動換行**（2026-08-04 新增，`ov_wrap`，t=3.5s，`maxWidth` 0.7、
    中英混排長標題）：成品 `x0=192 y0=395 w=696 h=212`（＝真的折成三行）→ 預覽
    `x0=192.0 y0=396.0 w=696.1 h=211.0`，**最大差 1.0px**。這是本檔第一個**多行 + CJK**
    的 case——「沒有多行也沒有 CJK」本來是這支腳本自己寫著的已知限制。
    ⚠️ **這一項的兩邊比對抓不到「換行沒實作」**：文字 overlay 的預覽與成品吃的是
  ...
  - ✅ **掛在畫布上緣外的 overlay**（2026-08-04 新增，`ov_offtop`，t=4.5s，`position.y = -0.03`）：
    ...
    「中心留在畫布內」）之後才可能出現的**負 `position.y`**——它當時只有手動驗過一次，
    沒有任何自動化守著。⚠️ 另外釘住成品側的形狀（`y0 ≤ 0`、`h ≤ 50`），因為「渲染端把負 y
    夾成 0」這種回歸要靠成品自己的樣子才抓得穩；實測把 fixture 的 y 翻正（模擬被夾成 0）
    會當場失敗（`y0=71 h=74`）。**下緣/左緣/右緣刻意不重複**（座標仍為正、同一條裁切路徑），
    **角落也刻意不用**：stage 的 `borderRadius: 10` 換算回畫布約 17px > 容差 4，會製造假警報。
- **所以這支腳本現在是綠的（exit 0）**。任何一項轉紅都是真的回歸：字幕那項紅＝量測本身壞了，
  先修腳本不要改斷言；overlay 四項紅＝幾何落差回來了（先看 `measure/` 裡的 PNG）。
...
- **已知限制**：只比「墨跡外框」（位置與尺寸），不比字形、不比顏色、不比 alpha 邊緣，
  ...
  非 karaoke 字幕與文字 overlay（含一個多行 CJK 換行 case），**karaoke 與純圖 overlay
  沒有 case**；每個時間窗只放
...
4. **打字體感（階段 3）**：在**右上字幕列表雙擊**一句字幕改字（三段式只接在 `CaptionList.tsx`，
   **畫布上不能直接改字**；左欄 Inspector 的字幕 Text 欄仍然不走三段式，但**已經不再每一鍵
   ...
   實測 33 個字 → `derived/text/` 多 99 個檔，而那個目錄只增不減］。
   換句話說「打字時畫布即時跟著變」只有右上字幕列表那條路，Inspector 是「改完離開欄位才生效」），
   感受「近似文字先出、~80ms 後換真卡」這個切換是否明顯/突兀。
   ✅ **「加長文字讓它自動換行」現在可以拿來驗了**（2026-08-04 起，見「文字自動換行」節）：
