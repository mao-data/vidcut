...
| M4 渲染                                   | ✅ `m4-done`                                   | ffmpeg 從 project.json 輸出成品 + 進度 + UI 渲染鈕（尺寸吃 `doc.canvas`；2026-08-26 多比例畫布上線前寫死 1080×1920）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 字幕 WYSIWYG 階段 1–4                     | ✅（分支 `caption-wysiwyg`，**四階段全完成**） | 階段 1：Pillow 常駐光柵器 + 字型表 + 字卡快取服務 + 字幕卡 debounce 同步。階段 2：**可編輯文字 overlay**——UI 時間軸「Text」鈕新增文字 overlay、Inspector 可改文字/字級/顏色，MCP `add_overlay`/`update_overlay`/`set_overlays` 皆支援 `text`。階段 3：**字幕預覽改走與成品同一張字卡**（**限非 karaoke**）——`fontSize/3` 估算已廢除，改用畫布座標空間 + `transform: scale(stage寬/畫布寬)`（當年寫死 1080×1920，2026-08-26 多比例畫布起改讀 `doc.canvas`）；karaoke 用 base+全高亮兩張幾何相同的卡疊 `clip-path` 逐詞揭色（**但匯出端是一詞一卡，兩者不是同一張圖**，見下節）；打字三段式即時預覽；真瀏覽器實測驗證縮放公式（見下節，注意那份量測不等於「預覽與成品對齊」）。階段 4：**畫布直接拖曳 overlay/字幕 + 吸附導線**——`shared/src/snap.ts` 的 `snapBBox` 純函數（水平置中/垂直置中/上下 5% 安全邊距，16px 吸附半徑）+ `ui/src/player/dragLayer.ts` 的錨點↔bbox 換算 + `Player.tsx` 的 pointer 拖曳與「放手後 echo 未到前」的本地覆蓋橋接；真瀏覽器 e2e 回歸（`npm run verify:canvas`，見下節）過程中抓到並修掉一個真 bug（`<img>` 原生瀏覽器拖曳手勢劫持 pointer 事件序列，見下節與 `.claude/rules/ui-verification.md`）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 跨專案素材庫（phase 1 後端 + phase 2 UI） | ✅（**後端＋MCP＋Media 面板 UI 全部完成**）    | spec `docs/superpowers/specs/2026-08-21-asset-library-design.md`（歷史文件，2026-08-25 補記已收斂導言與白名單的矛盾）。內容定址存放於 `~/.vidcut/library/`（`VIDCUT_LIBRARY_DIR` 可覆寫），`server/src/libraryStore.ts`／`libraryIngest.ts` 見上方檔案職責。五條 `/api/library*` HTTP 路由（上傳走串流、庫未載入時 503）＋四支 MCP 工具 `list_library`／`add_to_library`／`import_from_library`／`update_library_asset`（`remove_from_library` 刻意只給 HTTP DELETE，MCP 不開放，spec 決策）；工具數 34→38，instructions 已同步。測試：`libraryStore.test.ts`（10）、`libraryIngest.test.ts`（12）、`import-from-library.test.ts`（6）、`library-api.test.ts`（10）、`library-mcp.test.ts`（5）。**素材 UI 已落地**（`ui/src/panels/MediaPanel/`：`ProjectMediaZone.tsx`／`LibraryZone.tsx`）——Project（搜尋/上傳檔案與資料夾/列出/加入時間軸/存入素材庫）、Library（搜尋/上傳/匯入/改標籤/刪除確認）皆為真身，零 `useSelection.select()`（避免踢出分頁，見面板檔頭註解）。**2026-08-26 使用者定案的 UI 改版**：右欄改為扁平四分頁 Project/Library/Captions/Properties（去掉 Media 殼與子分頁列）、去框文字分頁、卡片網格（直式素材補黑邊 letterbox、欄數隨欄寬 auto-fill）、專案素材上傳走新的 `POST /api/media`；`SourceFolderZone.tsx` 保留但未掛載。**圖片走 `kind:'image'` 入庫、匯入專案走 image overlay**（非 clip track；`server/src/mcp.ts` 的 `import_from_library`／`add_overlay` 描述已同步）。**已知限制**：(1) `IMAGE_EXTENSIONS` 刻意不併入 `MEDIA_EXTENSIONS`（`server/src/sourceFolder.ts`）——素材夾掃描與 `import_media` 只認影音，Source folder zone 因此**列不出圖片檔**；圖片入庫走 Library zone 的上傳鈕，或 AI 的 `add_to_library`。(2) **svg 素材可入庫但不得匯入為 overlay**（`server/src/libraryIngest.ts` 的 `importImageToProject`）——ffmpeg 對 overlay 直接 `-i` 餵檔、多數 build 無 svg decoder，匯出會炸，於是在匯入這一步就擋下；HTTP／MCP 共用同一支 guard，`LibraryZone.tsx` 的 Import 鈕對 svg 列 disabled。真瀏覽器回歸 `npm run verify:panels` 已加素材 cases（四顆主分頁鈕 + Project 搜尋框/上傳檔案鈕/上傳資料夾鈕 + Library 搜尋框/上傳鈕命中測試）。 |
| MCP 面補完                                | ✅                                             | **每個工具都宣告 `outputSchema`**，由 `server/test/mcp-tools.test.ts` 的「批 G」守著（工具**數量**以 `server/src/mcp.ts` 為準——那條測試是 `toBeGreaterThanOrEqual`，下限不是釘死值，所以這裡不寫死）。`m3-done` 15 個 → 當時 31 個（用跨行安全的正則對兩個版本的 `mcp.ts` 各數一次 `registerTool` 得出，見 README 的九組工具表——**這是那次快照的數字，不隨後續分支更新；現況以 `mcp-surface-snapshot.test.ts` 鎖住的 39 為準**——2026-08-26 多比例畫布加了 `set_canvas`），新增的 16 個是 `list_source`／`add_clip`／`update_overlay`／`add_overlay`／`remove_overlay`／`remove_audio`／`update_caption`／`redo`／`transcribe`／`auto_caption`／`timeline_op`／`extract_audio`／`set_audio`／`update_audio`／`set_canvas_fit`／`set_cover`（`import_media` **M3 就有**，之後改的是語意——擴充成絕對路徑零複製引用）。「工具有沒有漏註冊、instructions 有沒有跟上」由 `server/test/mcp-docs-sync.test.ts` 守著（鐵則第三步的執行面）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 多比例畫布                                | ✅（2026-08-26）                               | 畫布不再寫死 1080×1920：`shared/src/canvasPresets.ts` 四檔 preset（portrait／landscape／square／portrait-4-5，**直式仍是預設**）＋ `setCanvas` 命令（`server/src/commands.ts`：preset 白名單、送現值 no-op 早退、換寬之後每張字卡的預算安全網，全有全無）＋ `server/src/canvasCards.ts` 的改畫布善後（重烤字幕＋重解析文字 overlay ＋清孤兒，由 `wsHub.ts` 精確認 `canvas/width` 與 `canvas/height` 兩格觸發，`canvas.fit`／`fps` 刻意不觸發）。UI 座標鏈（`Player.tsx`／`CaptionLayer.tsx`／`dragLayer.ts`／`CaptionList.tsx`）改讀 `doc.canvas`、不再吃 1080/1920 常數；Inspector 有 preset 入口、ExportMenu 的解析度選項改成倍率。MCP 工具 `set_canvas`（鐵則第三步已做）。`verify:wysiwyg`／`verify:canvas` 吃 `VIDCUT_CANVAS`；**wysiwyg 現在是直式＋橫式雙基線**（各 6/6 全綠，最大差 1.0px／1.2px，容差 4；square 與 portrait-4-5 沒跑過基線）。測試 `shared/src/canvasPresets.test.ts`／`server/test/canvasCards.test.ts`／`server/test/commands-t1.test.ts` 的 setCanvas describe。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
shared/src/proxyPlan.ts   proxyPlan：ingest A2（proxy）判準純函數，只吃 probe 靜態欄位；三種結果
                          ...
                          量不出來——保守原則）
shared/src/canvasPresets.ts CANVAS_PRESETS（四檔畫布比例：portrait 1080×1920／landscape 1920×1080／
                          square 1080×1080／portrait-4-5 1080×1350，陣列順序即 UI 顯示順序，portrait
                          必須是第一筆＝現狀行為的錨點）＋ `findCanvasPreset(w, h)` 反查純函數。
                          ⚠️ **比尺寸不比名字**——專案檔存的是展開後的 width/height，不存 preset id
                          （與 pro 的字幕 preset 同一紀律），日後調數值不會靜默改掉既有專案。
                          width/height 一律偶數：h264 不吃奇數尺寸，奇數 preset 會拖到 render 才炸
...
server/src/app.ts         Express app：`/api/project`（debug）、`GET /api/source?dir=`（素材夾掃描）、
                          `POST /api/import`（零複製匯入，逐支序列處理）、`POST /assets`（UI 上傳）、
                          `POST /api/media?name=`（**專案素材串流上傳**：body 直接是檔案位元組，
                          先落地 `.upload-<nanoid><ext>` 暫存再 rename 進 `media/`（同檔案系統原子搬移、
                          重名自動編號），接著 `ingestMedia` 登記＋背景衍生；probe 失敗會清掉孤兒檔並回 400。
                          去重看的是**解析後路徑**不是內容 hash——同一支檔案重傳會變成新的一筆）、`/media/*`、
...
server/src/canvasCards.ts 改畫布**尺寸**之後的字卡善後協調器（多比例畫布 Task 3）。畫布寬進字卡的
                          內容定址 key，`setCanvas` 一成立整套字卡的 hash 全變。`refreshCardsForCanvas()`
                          串三步：`CaptionCardSync.runNow()` 重烤字幕 → `refreshTextOverlayCards()` 重解析
                          文字 overlay（會改寫 doc 的 imagePath）→ `sweepOrphanCards()` 清孤兒。
                          ⚠️ **順序不能反**：清理是「刪掉沒被任何 imagePath 指到的檔」，重烤沒跑完的話
                          掃出來的集合毫無意義、還會刪掉正要重用的卡。所以字幕走 `runNow()` 不走
                          debounce 的 `schedule()`（後者沒有完成訊號可以等）。孤兒只掃 `derived/text/`
                          （`derived/captions/` 是 render 時以 caption id 覆寫的，不是內容定址、不歸這裡管），
                          認不得的檔名一律保留，刪除失敗逐檔吞掉只 warn——刪掉是安全的：內容定址，
                          undo 換回舊畫布時舊 hash 會原封不動算回來，檔案不在就當快取未命中重畫。
                          觸發點在 `wsHub.ts` 的 patch 監聽器，條件精確到 `canvas/width|height` 兩格；
                          `canvas.fit`／`canvas.fps` **刻意不觸發**（不影響字卡像素，放寬成 `canvas` 會讓
                          每次切填充方式都白跑一次全量重烤＋孤兒掃描）
...
ui/src/panels/MediaPanel/ 右欄的素材兩分頁（asset library phase 2 UI，見上方里程碑表）。
                          **2026-08-26 起沒有 `index.tsx` 分頁殼**：Project／Library 直接是 App.tsx
                          右欄四分頁中的兩顆（Project/Library/Captions/Properties），舊的兩層
                          分頁列已拆掉。`ProjectMediaZone.tsx`（搜尋、上傳檔案／上傳資料夾
                          → `POST /api/media`、列出當前專案素材、加入時間軸、存入素材庫）／
                          `LibraryZone.tsx`（搜尋 300ms debounce、上傳、匯入、雙擊改標籤、
                          刪除需二次確認）。兩者都用 `.media-grid` 卡片網格（16:9 縮圖、直式
                          素材 letterbox 補黑邊、操作鈕 hover 才浮現，見 `ui/DESIGN.md`）。
                          `SourceFolderZone.tsx` **保留但未掛載**（掃描路徑／勾選批次匯入，
                          仍是列表樣式；被 Project 分頁的上傳鈕取代，復活方式見該檔檔頭）。
                          三檔皆零 `useSelection.select()`——App.tsx 的「選取跳 Properties」
                          effect 會把使用者踢出本分頁，面板刻意繞開它。
