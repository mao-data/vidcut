...
> 相同；overlay 於 2026-08-04 修好兩個幾何落差，`npm run verify:wysiwyg` 四項全綠、最大差 1.0px）。
...
- **所以這支腳本現在是綠的（exit 0）**。任何一項轉紅都是真的回歸：字幕那項紅＝量測本身壞了，
  先修腳本不要改斷言；overlay 三項紅＝幾何落差回來了（先看 `measure/` 裡的 PNG）。
...
6. **預覽=成品的最終檢驗（階段 4 收尾）**：把上面拖過的 overlay/字幕、改過的文字，用同一次渲染比對。
   **請帶著下面這份「已知會不一致」的清單去看**，否則你會把已知落差當成新 bug（或反過來，
   ...
   - ✅ **overlay 的大小/位置現在也應該一樣**（2026-08-04 修好，`verify:wysiwyg` 四項全綠、最大差 1.0px）：
