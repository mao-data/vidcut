...
- ✅ **字幕(無逐詞高亮)**：預覽與匯出走同一支 `text_card.py`、同一份參數，輸出 PNG
  **逐位元組相同**（sha256 相等）。「涵蓋超寬文字、內嵌換行、未知字型、非 1080 畫布寬」
  出自 `docs/superpowers/specs/2026-08-03-caption-wysiwyg-design.md` 的一次性覆核
  （某次實測，未重驗）；常駐測試釘住的是**非 1080 畫布寬**（`server/test/textCards.test.ts`
  的 width 500，兩式 margin 分岔處）與**匯出與預覽走同一個字型檔**
  （`server/test/render-fonts.test.ts`，含「不注入 resolver 就必須不同」的判別性斷言）。
...
- ⚠️ **karaoke 字幕**：預覽是「base 卡 + 全高亮卡疊 `clip-path`」，匯出是「一詞一卡」，
  ...
  **結論先講，免得下一個人跑去修不存在的 bug：匯出那條路是正確的**（一詞一卡＝單層，
  ...
  成因 (b) 是主要貢獻者。**這組數字（含下面那個 10–19 像素）是某次實測，未重驗**——
  repo 內沒有常駐測試或腳本會重新產生它們，要引用先自己重跑。
...
- ⚠️ **`cardBudget.ts` 的行數估算取兩個上界的較小值**（`estimateCard`）：一是「每字元各佔
  一行」的**字元數上界**（與字型無關，永遠成立），二是 `greedyLineBound()` 的**前進寬上界**
  （每字元最寬 `MAX_ADVANCE_EM` = 3 em）。Node 側沒有字型量測，只能取上界——代價是很長的
  文字仍會被誤拒，但只有字元數上界的年代誤拒得更兇：1080 寬、fontSize 64 的實務上限只有
  **146 字**（一段 163 字的中文散文就被擋下），補上前進寬上界之後同樣條件是 **369 字**。
  字元數上界在「可用寬只放得下一個字」時會被真的打到（`textCards.test.ts` 釘住等號），
  不是保險係數；同一份測試也釘住 146 → 369 這個轉變。
