...
## 6. 暗版世界觀:剪接室暗房(2026-08-16 定案,取代 §2 的「紫世界身分不變」)

§2 的暗房調和是「調和,不重造」;紙桌面 craft 落地後使用者要求「暗版也要符合
主題」,經兩輪 mock 定案(`dark-worlds.html` B/C/D 三方向選 **C 剪接室暗房**,
`dark-c-variants.html`+`dark-c4-variants.html` 逐輪收斂為 **C4-d**):

- **世界**:傳統剪接師的桌。中性炭黑階梯(bg `#1a1a1c`/panel `#202023`/
  popover `#2a2a2e`;stage `#131315` 恆為最暗)、**白蠟筆**(grease pencil,
  `#e8e4da`)做文字/描邊/主鈕、**紅蠟筆**(`#c94f42`)只做標記——playhead
  (實心,漸層被對比實算否決:壓深端對 card 2.60 不合規)、時間碼進行值、
  當前字幕列左標(inset box-shadow 實作,避免 border 擠位)、時間軸選中紅框
  (`--select-edge`,與主行動 `--accent` 語意分家)、chip 紅描邊。
- **兩主題=同一套 Two-Hands 換支筆**:紙上鉛筆/紅鉛筆 ⇄ 暗房白蠟筆/紅蠟筆;
  `--who-ai`=蠟筆白 dim、`--who-you`=紅蠟筆(鏡像紙版)。
- **紫/青全面退役**(全庫 grep 清剿;`waveform.ts` 字面值為 jsdom 回退刻意保留,
  有測試釘);音訊軌降飽和藍灰(磁帶/技術邊欄);brand 字樣 chalk 單筆輕重。
- **使用者收斂定案**:對比降一格(片段框 .45 級)、無膠卷齒孔、無 header 粗線
  (沿用「app≠landing」體例);AgentStrip 琥珀終端一個 token 不動——炭黑裡
  唯一一盞終端小燈,信號稀缺性是刻意的。
- `--danger` 提亮至 `#fb8a8a` 與紅蠟筆三軸區辨(色相/飽和/亮度比 1.36);
  順手修掉白字對 `--approve-2` 3.31 的既有缺陷(炭黑字 4.61)。
- 驗收:paper 逐像素零變化(CDP diff=0)、對比全實算 ≥4.5、345 UI 測試斷言
  零修改、gauntlet 全數通過。
