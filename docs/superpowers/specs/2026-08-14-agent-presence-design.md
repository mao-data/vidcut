...
> **修訂(2026-08-16,使用者定案 + 階段 4 實作)——載體 A 案、卡擺正、署名不用手寫體。**
>
> 上面四項的**功能全部不變**(三態接線、實時行、最近 3 筆、讀數列、離線指令),
> 改的是載體、體例,以及一條在實作時發現的既有缺陷。
>
> **一、載體＝A 琥珀終端卡**(與 §3.3 的標籤同族,不是紙)。理由與標籤那次換裝
> 逐字相同——紙在暗房是一塊會發光的白斑;識別是**那隻手**(手繪圈 + `#ap-pencil`)。
> `RING_PATH` 從 `AgentStrip.tsx` **import 共用**而不是複製:同一支筆畫的同一個圈,
> 複製一份就會默默分岔,而 jsdom 不比對 path 形狀、沒有任何測試抓得到。
> `#ap-pencil` 的 `defs` 由標籤內嵌,SVG filter id 是 document 級可及,卡不再宣告一份。
>
> **二、卡本體擺正(無 rotate)**——這是與標籤刻意不同的地方。標籤是 header 上
> 26px 高的小物件,−0.6deg 讀成「貼上去的」;索引卡是多行文字的資料區塊,
> `ui/DESIGN.md` 的「Don't rotate a UI element」明列 data rows 不得旋轉
> (歪掉的多行文字讀成版面壞掉,不讀成手貼)。**app ≠ landing 的體例在這裡優先。**
>
> **三、上面「AI=graphite、you=紅鉛筆」與「卡外 Activity 維持紫/藍」兩句已過時。**
> 紫/藍在 2026-08-14 的雙主題那輪就全域退役,兩邊現在都吃同一組 `--who-*`
> (暗版 AI=白蠟筆 dim `#bdb8ac` / you=紅蠟筆 `#da7565`;紙版 AI=graphite /
> you=紅鉛筆)。卡內卡外**同一個顏料**,不再分家。
>
> **四、署名 `—AI`/`—you` 不用手寫體**(A 案):吃跟左邊那格同一個 `--who-*`,
> 只降字級 11 → 10px。**不用 opacity 降階**——實算 `--who-you` 在 α0.8 下對
> `--panel` 只有 3.81:1(滿版 5.20),一降就掉出語意文字門檻。DESIGN.md 原本
> 「Caveat 留給索引卡」的保留條款因此**用不到了**,已同步改寫成無指定消費者的儲備。
>
> **五、卡與面板只能靠線分開**:`--ap-slate` 對 `--panel` 是 1.01:1、紙版
> `--ap-paper` 對紙面板是 1.00:1(同一個字面值)。所以邊界是**結構性 1px 實線**,
> 四種組合全部實算過非文字 3:1(idle 3.12 暗 / 3.07 紙;offline 3.42 / 3.96),
> 且 offline **換顏料不是降透明度**(α 壓到 0.12 只有 1.21:1,那不是暗下去,是邊界消失)。
>
> **六、補一個既有缺陷(行為變更,併入本包)**:設定選取有九條路徑,清除只有三顆
> 刪除鈕——**選過任何東西之後就永遠回不到 Inspector 的閒置區**,而索引卡就住在
> 那裡(「未選取才顯示」的機制沒變)。補兩條回頭路:
>
> - `Escape`(`ui/src/App.tsx` 的 onKey;打字中不作用沿用該 handler 既有的
>   INPUT/TEXTAREA/contentEditable 守衛,帶修飾鍵沿用 `if (mod) return`)。
>   **`ui/src/shortcuts.ts` 已同步**(CLAUDE.md 鐵則,Shortcuts 彈出層由它生成)。
> - **點時間軸空白處**(`ui/src/timeline/Timeline.tsx`):掛在內容層、靠
>   `data-tl-blank` **白名單**分辨,不是 `e.target === e.currentTarget`——「空白」
>   不只有內容層自己,四條軌道各自的空處(chip 右邊那一大片)才是使用者最常點的
>   地方,而那些空處在 DOM 上屬於軌道 row。反過來,clip / chip / 把手 / **尺規**
>   全都冒泡到同一層,尺規的 click 是 seek 絕不能被搶,所以標記是明確 opt-in:
>   只有內容層與四條軌道 row 掛,其餘一律不掛。
