...
> **歷史文件（2026-08-03 的設計定案）**：記錄當時的決策與理由，**不隨程式碼更新**。
...
- 根目錄白名單：server 綁 127.0.0.1，且限制會擋到使用者自己的素材。
  只回白名單副檔名、排除隱藏檔（`dir` 本身不檢查 `..`——`GET /api/source`／
  `POST /api/import` 的 `dir` 參數本來就吃任意絕對路徑，`..` 不會多給權限，
  沒有安全後果；見 `server/src/app.ts:28-46`）
- 素材夾路徑持久化：先每次輸入
